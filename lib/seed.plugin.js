    ...
    return waterfall(
      [
        (next) => {
          ...
          return fetchDependency(ignore, next);
        }, // fetch ignored
        (ignored, next) => {
          // use ignored
          ...
          let { model, match, array } = mergeObjects(dependency);
          if (!_.isEmpty(ignoredIds)) {
            match = mergeObjects({ [ignorePath]: { $nin: ignoredIds } }, match);
          }
          return fetchDependency({ model, match, array }, next);
        }, // fetch dependencies exclude ignored
      ],
      afterDependency
    );
