 ...
 * @description Parse paths found in unique mongdb error.
 * @param {Error} valid mongodb error.
...
const handleUniqueError = schema => {
  ...
  const normalizeUniqueError = (error, doc, next) => {
    ...
    value = value ? value.match(/"(.+?)"/g) : value; // enclosed with quotes
    value = value.map(v => v.replace(/^"(.+?)"$/, '$1')); //replace quotes
    value = paths.length === 1 ? [value.join(' ')] : value;
